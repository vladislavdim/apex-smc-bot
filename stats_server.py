...
def _fetch(days: int, strategy: str, symbol: str) -> list[dict[str, Any]]:
...
def build_dashboard(days: int = 1, strategy: str = "", symbol: str = "", outcome: str = "", groq: str = "",
    ...
    return {"period_days":days,"generated_at":datetime.now(timezone.utc).isoformat(timespec="seconds"),
      ...
      "criterion_stats":[{"label":k,"pass":v.get("PASS",0),"fail":v.get("FAIL",0),"total":v.get("PASS",0)+v.get("FAIL",0)} for k,v in sorted(checks.items(),key=lambda x:sum(x[1].values()),reverse=True)[:200]],
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path=="/api/dashboard":
            try:
                val=lambda k,d="":(q.get(k) or [d])[0]; data=build_dashboard(int(val("days","1")),val("strategy"),val("symbol"),val("outcome"),val("groq"),float(val("min_rr")) if val("min_rr") else None,float(val("max_rr")) if val("max_rr") else None,int(val("page","1")),int(val("page_size","100"))); self._json(data)
