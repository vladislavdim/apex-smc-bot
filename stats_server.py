...
function renderResearchAttemptDetails(){
 ...
 const attempts=d.attempts||[],trades=d.attempt_trades||[],byAttempt=trades.reduce((m,x)=>{(m[x.attempt_id]||(m[x.attempt_id]=[])).push(x);return m},{});
 const body=attempts.map((a,i)=>{const linked=byAttempt[a.attempt_id]||[],t=linked.find(x=>x.track==='PLAYBOOK_ONLY')||linked[0]||{};return '<tr><td>'+new Date(Number(a.decision_time)*1000).toISOString().slice(0,19).replace('T',' ')+'</td><td><b>'+esc(a.parent_strategy)+'</b><div class=muted>'+esc(a.profile_id)+'</div></td><td>'+esc(a.symbol)+'</td><td>'+esc(a.direction)+'</td><td>'+esc(a.outcome)+'</td><td>'+num(a.entry)+'</td><td>'+num(a.sl)+'</td><td>'+num(a.tp1)+'</td><td>'+num(a.tp2)+'</td><td>'+num(a.rr)+'</td><td>'+esc(t.status||'—')+'<div>'+num(t.net_r)+'R</div></td><td><button class=btn onclick="toggleResearchAttempt('+i+')">детали</button></td></tr><tr class=details id=ra'+i+'><td colspan=12><div class=detailbox>'+esc(JSON.stringify({attempt:a,tracks:linked},null,2))+'</div></td></tr>'}).join('');
