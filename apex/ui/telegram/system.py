...
def format_system_status(snapshot: Mapping[str, Any]) -> str:
    ...
    generation = snapshot.get("fencing_generation")
    lines.append(f"🔐 Fencing generation: <b>{generation if generation is not None else '—'}</b>")
    lines.extend(["", "<b>Подключения и компоненты</b>"])
    ...
    for name in _ORDER:
        ...
        lines.append(f"{_icon(state)} {html.escape(label)}: <b>{html.escape(state)}</b>")
    ...
    if reasons:
        for reason in reasons[:8]:
            lines.append(f"• <code>{html.escape(str(reason))}</code>")
    else:
        lines.append("Нет.")
    lines.extend(["", "<i>Показывается каноническое состояние worker, без повторных внешних запросов.</i>"])
    return "\n".join(lines)[:4000]
