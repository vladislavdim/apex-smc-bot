...
from core.trade_execution import ExecutionConfig, execute_approved_candidate
...
class ApexV3RuntimeTests(unittest.TestCase):
    ...
    def test_manager_and_execution_have_no_runtime_replay_dependency(self):
        root = os.path.dirname(os.path.dirname(__file__))
        with open(os.path.join(root, "core/execution_ledger.py"), encoding="utf-8") as source:
            self.assertNotIn("from core.replay_lab import", source.read())
        with open(os.path.join(root, "core/trade_manager.py"), encoding="utf-8") as source:
            manager = source.read()
