...
def test_snapshot_manifest_is_complete_and_idempotently_verifiable(tmp_path):
    db, dashboard, db_gz, dashboard_gz = _artifacts(tmp_path)
    ...
    assert manifest["no_real_execution"] is True
    manifest_path = tmp_path / "BTCUSDT.manifest.json"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    assert verify_compressed_assets(db_gz, dashboard_gz, manifest_path)["symbol"] == "BTCUSDT"
    assert validate_database(db)["counts"]["15m"]["count"] == 1


def test_snapshot_rejects_paused_run_and_open_gap(tmp_path):
    db, dashboard, db_gz, dashboard_gz = _artifacts(tmp_path)
    ...
    with pytest.raises(SnapshotValidationError, match="run_not_completed"):
    ...
    with pytest.raises(SnapshotValidationError, match="open_history_gaps"):
        validate_database(db)
