...
def search_crypto_news(query: str) -> list:
    ...
    sources = [
        f"https://cointelegraph.com/search?query={requests.utils.quote((query or chr(32)).strip())}",
        f"https://cryptonews.com/search/?q={requests.utils.quote((query or chr(32)).strip())}",
    ]
    results = []
    for url in sources:
