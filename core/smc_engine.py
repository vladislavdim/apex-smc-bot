...
# ═══════════════════════════════════════════════════════════════
# GROQ ПИШЕТ СЕБЕ УЛУЧШЕНИЯ (пункт 4)
# ═══════════════════════════════════════════════════════════════

def groq_self_improve():
    """
    Groq анализирует свои результаты и сам генерирует улучшения:
    - Новые стратегии
    - Улучшенные правила
    - Идеи для новых функций (записывает как задачи)
    """
    try:
        conn = sqlite3.connect(DB_PATH)
        ...
        prompt = f"""Ты — AI торговый бот APEX. Проанализируй свои результаты и предложи конкретные улучшения.

СТАТИСТИКА (7 дней):
- Всего сигналов: {total}
- Win Rate: {wr}%
- Побед: {wins}, Поражений: {losses}
- Лучшие сделки (монета, R:R): {best}
- Худшие сделки (монета, confluence): {worst}
- Правил в базе: {rules_count}

ЗАДАЧА: Придумай 3-5 конкретных улучшения которые повысят Win Rate.
Каждое улучшение должно быть:
1. Конкретным (не "улучшить анализ", а "добавить фильтр по объёму перед входом")
2. Реализуемым через правила/стратегии
3. Основанным на реальных данных выше

Ответь в JSON:
{{
  "improvements": [
    {{
      "type": "new_rule|strategy|filter|timing",
      "title": "название улучшения",
      "description": "что именно изменить",
      "rule_text": "текст правила для self_rules или null",
      "rule_type": "prefer|avoid|timing|risk",
      "confidence": 0.0-1.0,
      "priority": 1-10
    }}
  ],
  "insight": "главный вывод о текущих слабостях"
}}"""

        response = _groq(prompt, max_tokens=800)
        if not response:
            return []
        ...
        if insight:
            conn.execute("""INSERT INTO web_knowledge (topic, query, summary, source_url, relevance)
                VALUES (?,?,?,?,?)""",
                ("self_improvement", "groq_self_analysis", insight, "internal", 0.9))

        # Сохраняем правила из улучшений
        saved = 0
        for imp in improvements:
            rule_text = imp.get("rule_text")
            if rule_text and imp.get("confidence", 0) >= 0.65:
        ...
        logging.info(f"[WebLearner] 🧠 Groq самоулучшение: {len(improvements)} идей, {saved} правил добавлено. Инсайт: {insight[:80]}")
        return improvements
