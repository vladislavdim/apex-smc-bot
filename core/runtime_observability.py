...
def _patch_stats_html(html: str) -> str:
    ...
    new_tabs = '<div class=tabs id=periods><button class="btn active" id=currentRelease disabled>Stable baseline</button></div>'
    ...
    operational_js = r'''function renderOperational(){const md=LAST.market_data||{rows:[]},mr=md.rows||[],obs=LAST.system_overview||{};marketData.innerHTML=`<div><b class=good>Gate requests OK: ${md.ok||0}/${md.total||0}</b> · <b class="${md.failed?'bad':'muted'}">Failed: ${md.failed||0}</b> · <b class="${md.stale?'warn':'muted'}">Stale TF: ${md.stale||0}</b> · <b class="${obs.observability_state==='COMPLETE'?'good':'warn'}">Observability ${esc(obs.observability_state||'UNKNOWN')}</b></div><div class=muted>OK means the latest request succeeded; candle freshness is measured from the last closed venue candle. ${esc((obs.observability_reasons||[]).join(', '))} · Last update: ${esc((md.last_update||'—').replace('T',' ').slice(0,19))} UTC</div>`+mr.slice(0,20).map(x=>{const fresh=x.freshness_status||'UNKNOWN',ok=x.status==='OK',cls=!ok?'bad':fresh==='FRESH'?'good':'warn';return `<div class=crit style="margin-top:7px"><b>${esc(x.symbol)} · ${esc(x.timeframe)} · <span class="${cls}">${ok?'✅':'❌'} ${esc(x.source)} · ${esc(fresh)}</span></b><div>${x.reason?esc(x.reason):'candles '+esc(x.candle_count)}</div><div class=muted>last closed ${esc((x.last_closed_candle_at||'—').replace('T',' ').slice(0,19))} · request ${esc((x.last_success_at||'—').replace('T',' ').slice(0,19))} · age ${x.age_seconds==null?'—':Math.round(x.age_seconds/60)+'m'} · update ${esc((x.last_update_at||'—').replace('T',' ').slice(0,19))}</div></div>`}).join('');const lw=LAST.ltf_watch||{rows:[]};ltfWatch.innerHTML=`<div><b>Unique waiting setups: ${lw.waiting||0}</b></div>`+(lw.rows||[]).map(x=>`<div class=crit style="margin-top:7px"><b>${esc(x.strategy)} · ${esc(x.symbol)} · ${esc(x.required_timeframe||'—')}</b><div>${esc(x.reason||'')}</div><div class=muted>setup ${esc((x.setup_id||'legacy').slice(0,10))} · attempts ${esc(x.attempts||0)} · misses ${esc(x.misses||0)} · expires ${esc((x.expires_at||'—').replace('T',' ').slice(0,19))}</div></div>`).join('');const sw=LAST.swing_volume_shadow||{},ft=LAST.fast_target_diagnostics||{};if(sw.observed)numericDiag.innerHTML+=`<div class=crit style="margin-top:8px"><b>SWING volume shadow 1.2x → 1.1x</b><div>n=${sw.observed} · current ${sw.pass_1_2} · shadow ${sw.pass_1_1} · shadow-only ${sw.shadow_only}</div></div>`;if(Object.keys(ft).length)numericDiag.innerHTML+=`<div class=crit style="margin-top:8px"><b>FAST target diagnostics</b><div>${Object.entries(ft).map(([k,v])=>esc(k)+' '+v).join(' · ')}</div></div>`}'''
    html = html.replace("function renderNumericDiag(){", operational_js + "function renderNumericDiag(){")
    html = html.replace("test_count:'ZONE test count'", "test_count:'ZONE prior test count (passes at ≤2)'")
    html = html.replace("renderNumericDiag();bars(LAST.failures", "renderNumericDiag();renderOperational();bars(LAST.failures")
    html = html.replace(
        "const old=LAST.wyckoff_dist_range||{},box=LAST.wyckoff_box_width||{},sh=LAST.wyckoff_shadow||{};",
        "const old=LAST.wyckoff_dist_range||{},box=LAST.wyckoff_box_width||{},sh=LAST.wyckoff_shadow||{},acc=LAST.wyckoff_accumulation_shadow||{};",
    )
    html = html.replace(
        "old-only ${sh.old_only}</div></div>`:'';wyCompare.innerHTML=`<div class=criteria>${fmt('Старый 30d high-low',old)}${fmt('Новый BC/AR/ST box',box)}${shadow}</div>`",
        "old-only ${sh.old_only} · UTAD+SOW ready ${sh.phase_ready||0}</div></div>`:'';const ashadow=acc.observed?`<div class=crit><b>Accumulation shadow @ 25%</b><div class=muted>n=${acc.observed}</div><div>old ${acc.old_pass} · structural ${acc.structural_pass} · structural-only ${acc.structural_only} · Spring+SOS ready ${acc.phase_ready||0}</div></div>`:'';wyCompare.innerHTML=`<div class=criteria>${fmt('Старый 30d high-low',old)}${fmt('Новый BC/AR/ST box',box)}${shadow}${ashadow}</div>`",
    )
    ...
    render_marker = "function render(){updated.textContent='Обновлено '+LAST.generated_at.replace('T',' ').slice(0,19)+' UTC'+(LAST.release_sha?' · '+LAST.release_sha.slice(0,8):'');summary.innerHTML=cards(LAST.summary);renderV2();renderFunnels();renderBosAge();renderWyCompare();renderNumericDiag();"
    render_repl = "function renderFastTiming(){const ft=LAST.fast_stage_timing||{},m=ft.metrics||{},labels={liquidity_ms:'Liquidity',context_15m_ms:'15m context',htf_ms:'HTF 1h/4h',btc_ms:'BTC context',zone_4h_ms:'4h zone/context',trigger_ms:'15m trigger',total_pair_ms:'Total pair'};fastTiming.innerHTML=Object.entries(labels).map(([k,l])=>{const x=m[k]||{};return `<div class=crit><b>${l}</b><div class=muted>n=${x.count||0}</div><div>P50 ${num(x.median)} ms · P75 ${num(x.p75)} · P90 ${num(x.p90)}</div></div>`}).join('')||'<span class=muted>Нет данных</span>'}function render(){const runtime=((LAST.versions||{}).release_sha||LAST.runtime_release_sha||LAST.current_release_sha||'').slice(0,8),baseline=(LAST.baseline_utc||LAST.baseline_started_at||'').replace('T',' ').slice(0,19);cohortLabel.textContent=`Baseline since ${baseline||'—'} UTC · Runtime release ${runtime||'—'}`;updated.textContent='Обновлено '+LAST.generated_at.replace('T',' ').slice(0,19)+' UTC';summary.innerHTML=cards(LAST.summary);renderV2();renderFunnels();renderBosAge();renderWyCompare();renderFastTiming();renderNumericDiag();"
