...
def _patch_stats_html(html: str) -> str:
    ...
    operational_js = r'''function renderOperational(){const md=LAST.market_data||{rows:[]},mr=md.rows||[];marketData.innerHTML=`<div><b class=good>Gate candles OK: ${md.ok||0}/${md.total||0}</b> · <b class="${md.failed?'bad':'muted'}">Failed: ${md.failed||0}</b></div><div class=muted>Last update: ${esc((md.last_update||'—').replace('T',' ').slice(0,19))} UTC</div>`+mr.slice(0,20).map(x=>`<div class=crit style="margin-top:7px"><b>${esc(x.symbol)} · ${esc(x.timeframe)} · <span class="${x.status==='OK'?'good':'bad'}">${x.status==='OK'?'✅':'❌'} ${esc(x.source)}</span></b><div>${x.reason?esc(x.reason):'candles '+esc(x.candle_count)}</div><div class=muted>last success ${esc((x.last_success_at||'—').replace('T',' ').slice(0,19))} · update ${esc((x.last_update_at||'—').replace('T',' ').slice(0,19))}</div></div>`).join('');const lw=LAST.ltf_watch||{rows:[]};ltfWatch.innerHTML=`<div><b>Waiting: ${lw.waiting||0}</b></div>`+(lw.rows||[]).map(x=>`<div class=crit style="margin-top:7px"><b>${esc(x.strategy)} · ${esc(x.symbol)} · ${esc(x.required_timeframe||'—')}</b><div>${esc(x.reason||'')}</div><div class=muted>attempts ${esc(x.attempts||0)} · misses ${esc(x.misses||0)} · expires ${esc((x.expires_at||'—').replace('T',' ').slice(0,19))}</div></div>`).join('');const sw=LAST.swing_volume_shadow||{},ft=LAST.fast_target_diagnostics||{};if(sw.observed)numericDiag.innerHTML+=`<div class=crit style="margin-top:8px"><b>SWING volume shadow 1.2x → 1.1x</b><div>n=${sw.observed} · current ${sw.pass_1_2} · shadow ${sw.pass_1_1} · shadow-only ${sw.shadow_only}</div></div>`;if(Object.keys(ft).length)numericDiag.innerHTML+=`<div class=crit style="margin-top:8px"><b>FAST target diagnostics</b><div>${Object.entries(ft).map(([k,v])=>esc(k)+' '+v).join(' · ')}</div></div>`}'''
