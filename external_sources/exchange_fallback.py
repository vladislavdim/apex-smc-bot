...
async def collect(symbol: str) -> dict:
    async def fetch():
        ...
        # Binance/Bybit use scaled tickers for several low-price contracts,
        # while Gate uses the bot's unscaled base contract name.
        gate_contract = f"{symbol[:-4]}_USDT"
        premium, oi_1h, oi_4h, depth, bybit, gate_1h, gate_4h, gate_contract_info, gate_depth = await asyncio.gather(
            _safe(http_client.get_json(f"{binance}/fapi/v1/premiumIndex", {"symbol": alternative})),
            _safe(http_client.get_json(f"{binance}/futures/data/openInterestHist", {"symbol": alternative, "period": "1h", "limit": 2})),
            _safe(http_client.get_json(f"{binance}/futures/data/openInterestHist", {"symbol": alternative, "period": "4h", "limit": 2})),
            _safe(http_client.get_json(f"{binance}/fapi/v1/depth", {"symbol": alternative, "limit": 20})),
            _safe(http_client.get_json("https://api.bybit.com/v5/market/tickers", {"category": "linear", "symbol": alternative})),
            _safe(http_client.get_json("https://api.gateio.ws/api/v4/futures/usdt/contract_stats", {"contract": gate_contract, "interval": "1h", "limit": 2})),
            _safe(http_client.get_json("https://api.gateio.ws/api/v4/futures/usdt/contract_stats", {"contract": gate_contract, "interval": "4h", "limit": 2})),
