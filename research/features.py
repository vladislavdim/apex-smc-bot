...
def compute_feature_snapshot(symbol: str, timeframe: str, candles: list[Mapping[str, Any]], *,
    ...
    result = {
        ...
        "volatility": {"atr": atr, "atr_price_pct": atr/price*100 if atr and price else None,
                       ...
                       "compression_duration": sum(1 for x in reversed(ranges[:-1]) if x <= statistics.fmean(ranges[-20:])) if len(ranges)>=20 else None,
...
def levels_from_snapshot(snapshot: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for swing in structure.get("swings") or []:
        ...
        levels.append({"symbol":symbol,"timeframe":timeframe,"level_type":kind,"direction":"",
    ...
    if isinstance(ob, Mapping):
        ...
        if low > 0 and high >= low:
            levels.append({"symbol":symbol,"timeframe":timeframe,"level_type":"OB","direction":structure.get("direction") or "",
    ...
    if isinstance(fvg, Mapping):
        ...
        if low > 0 and high >= low:
            levels.append({"symbol":symbol,"timeframe":timeframe,"level_type":"FVG","direction":structure.get("direction") or "",
    ...
    for key in ("poc","vah","val"):
        ...
        if price > 0:
            levels.append({"symbol":symbol,"timeframe":timeframe,"level_type":key.upper(),"direction":"",
                           "low":price,"high":price,"created_at_ts":as_of,"status":"ACTIVE","attributes":{}})
    vwap = _number(location.get("vwap"))
    if vwap > 0:
        levels.append({"symbol":symbol,"timeframe":timeframe,"level_type":"VWAP","direction":"",
